...
int main() {
	...
	cerr << "BM_search: " << chrono::duration_cast<chrono::milliseconds>(dur).count() << " ms" << endl;
	...
	while (pos_it != text.end()) {
		...
		bool isOK = true;
		for (size_t i = from; i < min(text.size(), from + pattern.size()); ++i) {
	...
	dur = finish - start;
	cerr << "find_search: " << chrono::duration_cast<chrono::milliseconds>(dur).count() << " ms" << endl;
